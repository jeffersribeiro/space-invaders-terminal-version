...
import { ORIENTATION } from "@/engine/enums";
...
export class InputComponent extends Component {
  ...
  update(dt: number): void {
    ...
    const buttons = Controller.getController();
    ...
    if (this.vx !== 0 || this.vy !== 0) {
      ...
      this.sprite.play(`walk_${dir}`);
    } else {
      // idle
      const idleDir = ORIENTATION[this.orientation];
      this.sprite.play(`idle_${idleDir}`);
    }

    // 6) Example: handle roll or jump buttons
    if (buttons[GAMEPED_MAPPED_KEY.ACTION_ONE].pressed) {
      this.sprite.play(`roll_${ORIENTATION[this.orientation]}`);
      // apply a small nudge:
      const nudge = 0.2 * dt;
